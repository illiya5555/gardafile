/*
  # Fix Dashboard Statistics Refresh Function

  1. Changes
     - Fixed syntax error in refresh_dashboard_stats() function by replacing invalid 
       "REFRESH MATERIALIZED VIEW IF EXISTS" with proper conditional check
     - Re-implemented refresh logic for materialized views using PL/pgSQL conditional blocks
     - Maintained all other functionality from the original migration

  2. Indexes
     - Added performance indexes for reservations, time slots, testimonials, and inquiries
     
  3. Views
     - Created dashboard_stats materialized view
     - Created unified_dashboard_stats materialized view with detailed metrics
     
  4. Functions
     - Added search vector update functions for better text search
     - Created refresh function for dashboard statistics views
     
  5. Triggers
     - Added triggers to update search vectors
     - Added trigger to refresh dashboard statistics on data changes
*/
...
-- FIXED: Using proper conditional logic to check if view exists before refreshing
CREATE OR REPLACE FUNCTION refresh_dashboard_stats()
RETURNS TRIGGER AS $$
BEGIN
  -- Always refresh dashboard_stats
  REFRESH MATERIALIZED VIEW dashboard_stats;
  
  -- Conditionally refresh unified_dashboard_stats if it exists
  IF EXISTS (
    SELECT 1 FROM pg_matviews WHERE matviewname = 'unified_dashboard_stats'
  ) THEN
