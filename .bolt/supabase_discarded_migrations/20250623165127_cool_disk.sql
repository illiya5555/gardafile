/*
  ...
  1. Table Structure Improvements
    - Add email_sent column to reservations table for tracking email confirmations
    - Add booking_id column to stripe_orders table to replace stripe_checkout_metadata
    - Add several missing indexes for improved query performance

  2. Dashboard Stats Functions
    - Fix refresh_dashboard_stats trigger function to properly return TRIGGER type
    - Create materialized views for dashboard statistics
    - Set up proper triggers for refreshing stats when data changes
*/
...
-- 4. Drop and recreate dashboard stats function with proper TRIGGER return type
DROP FUNCTION IF EXISTS refresh_dashboard_stats();

CREATE OR REPLACE FUNCTION refresh_dashboard_stats()
RETURNS TRIGGER AS $$
BEGIN
  -- Refresh both materialized views for dashboard stats
...
-- 5. Create or replace materialized view for dashboard stats
...
-- 6. Create or replace unified dashboard stats view with more metrics
...
-- 7. Recreate trigger for dashboard stats refresh
...
-- 8. Update booking_after_payment trigger to use the booking_id in stripe_orders
...
-- 9. Create trigger for order status changes
...
-- 10. Create function for search vectors (if missing)
