...
-- Instead of using DEFAULT with subquery, create a trigger to set default role
CREATE OR REPLACE FUNCTION set_default_role_id()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.role_id IS NULL THEN
    NEW.role_id := (SELECT id FROM user_roles WHERE role_name = 'client');
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS set_default_role_id_trigger ON profiles;
CREATE TRIGGER set_default_role_id_trigger
